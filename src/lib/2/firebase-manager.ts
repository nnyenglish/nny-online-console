...
export class FirebaseManager {
	...
	public async batchEnd() {
		...
		if (this.batchCount > 0) {
			logger.info(
				`[${fnName}] batchCount == ${this.batchCount} => Run batch.commit()`
			);
			this.batchCount = 0;
	...
	public async createDoc(
		...
		if (getDocData.exists()) {
			logger.error(`[${fnName}] ${collectionPath}:${JSON.stringify(docData)}, 이미 존재하는 Doc`);
	...
	public async updateDoc(
		...
		try {
			if (bBatch) {
				...
				await FirebaseManager.runFirestoreAPI(fnName, collectionPath, () =>
					firebaseUpdateDoc(docRef, docData)
				);
			}
		} catch (error) {
			logger.error(`[${fnName}] 예외 발생, error = ${JSON.stringify(error, undefined, 2)}`);
	...
	public async getDoc<T>(docPath: string) {
		...
		try {
			...
			const documentSnapshot = await FirebaseManager.runFirestoreAPI(
				fnName,
				docPath,
				() => firebaseGetDoc(docRef)
			);
			...
			logger.error(
				`[${fnName}] 예외 발생, error = ${JSON.stringify(error, undefined, 2)}`
			);
			throw error;
		}
	}
	...
	public async getDocsArrayWithWhere<T>(
		...
		try {
			const querySnapshot = await this.querySnapshotWithWhere<T>(
				fnName,
				collectionPath,
				wheres,
				options
	...
	public observe<T>(
		...
		return new Observable<T[]>((subscriber) => {
			...
			const unsubscribe = () => {
				unsubscribeSnapshot();
				logger.info(`[${fnName}] unsubscribe: ${collectionPath}`);
			};
	 ...
	 */
	private static async runFirestoreAPI<T>(
		caller: string,
		collectionOrDocPath: string,
		api: () => Promise<T>
	) {
		const maxTry = 3;
		let countTry = 0;
		...
		while (countTry < maxTry) {
			try {
				...
				if (countTry > 1) {
					...
					logger.error(
						`[${caller}:${collectionOrDocPath}] countTry = ${countTry}`
					);
				}
	...
	private querySnapshotWithWhere<T>(
		...
		// 1. wheres를 적용
		const queryFn: QueryConstraint[] = wheres.map((_where) =>
			where(_where[0], _where[1], _where[2])
		);
